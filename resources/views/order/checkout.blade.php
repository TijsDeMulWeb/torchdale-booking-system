                            ...
                            <div id="items-grid" class="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
                                ...
                                @forelse ($products as $product)
                                    @php
                                        ...
                                        $hasShipping  = ($product->shipping_cost_domestic > 0 || $product->shipping_cost_international > 0);
                                    @endphp

                                    @if($hasVariants)
                                        {{-- Toon een kaart per variatie --}}
                                        @foreach($product->variants as $variant)
                                            @php
                                                $vPrice      = $variant->selling_price ?? $effectivePrice;
                                                $vStock      = $variant->stock_quantity;
                                                $vOutOfStock = !is_null($vStock) && $vStock <= 0;
                                                $vClick      = $hasShipping ? 'openProductModal(this)' : 'addToCart(this)';
                                            @endphp
                                            <button
                                                @if(!$vOutOfStock) onclick="{{ $vClick }}" @endif
                                                data-type="product"
                                                data-id="{{ $product->id }}_v{{ $variant->id }}"
                                                data-name="{{ $product->name }} – {{ $variant->name }}"
                                                data-price="{{ $vPrice }}"
                                                data-original-price="{{ $vPrice }}"
                                                data-discount-type=""
                                                ...
                                                <span class="text-xs text-gray-400 dark:text-gray-500 leading-snug">{{ $product->name }}</span>
                                        ...
                                        @endforeach
                                    @else
                                        {{-- Geen variaties: toon het product direct --}}
                                        @php
                                            $outOfStock   = !is_null($product->stock_quantity) && $product->stock_quantity <= 0;
                                            $clickHandler = $hasShipping ? 'openProductModal(this)' : 'addToCart(this)';
                                        @endphp
                                        <button
