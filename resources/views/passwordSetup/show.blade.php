    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-sm">
            <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900 dark:text-white">
                {{ __('auth.password_setup_title') }}
            </h2>
            <p class="mt-2 text-center text-sm/6 text-gray-500 dark:text-gray-400">
                {!! __('auth.password_setup_description', ['name' => '<strong class="text-gray-900 dark:text-white">' . e($user->first_name) . '</strong>']) !!}
            </p>
        </div>

        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form action="{{ url()->full() }}" method="POST" class="space-y-6">
                @csrf
