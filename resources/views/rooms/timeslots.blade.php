    ...
    <div class="px-4 sm:px-6 lg:px-8 my-10">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
            @foreach (['Maandag', 'Dinsdag', 'Woensdag', 'Donderdag', 'Vrijdag', 'Zaterdag', 'Zondag'] as $dayIndex => $day)
                ...
                <div class="grid grid-cols-[140px_1fr] {{ !$loop->last ? 'border-b border-gray-100 dark:border-white/10' : '' }}">
                    ...
                    <div class="py-4 px-5 border-l border-gray-100 dark:border-white/10">
                        <div class="flex flex-wrap gap-3 items-center">
                            @foreach ($daySlots as $slot)
                                <div class="inline-flex items-center gap-3 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300">
                                    <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    <span class="font-medium">{{ $slot->start_time->format('H:i') }} – {{ $slot->end_time->format('H:i') }}</span>
                                    <div class="flex items-center gap-2 ml-1 pl-3 border-l border-gray-200 dark:border-white/10">
                                        <button type="button"
                                        ...
                                        <form method="POST" action="{{ route('rooms.timeslots.destroy', [$room->id, $slot->id]) }}" class="inline">
                                            @csrf
                                            @method('DELETE')
        ...
        <div id="modal-overlay" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
            <div id="modal-box" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-white/10 w-full max-w-md p-8">
                <div class="flex items-center justify-between mb-6">
                    <h2 id="modal-title" class="text-base font-semibold text-gray-900 dark:text-white"></h2>
                    <button type="button" onclick="closeModal()" class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
